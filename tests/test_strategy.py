...
def _alloc(regime=MarketRegime.BULL, stable=0.2, max_lev=2.0, age_s=0.0):
    return StrategicAllocation(
        ...
        produced_at=datetime.now(UTC) - timedelta(seconds=age_s),
    )


def test_bull_regime_forbids_new_short_trends():
    lim = limits_for(
        _alloc(regime=MarketRegime.BULL),
...
def test_bear_regime_forbids_new_long_trends():
    lim = limits_for(
        _alloc(regime=MarketRegime.BEAR),
...
def test_chop_regime_forbids_any_trend_entry():
    for reason in [ReasonCode.ENTER_LONG_TREND, ReasonCode.ENTER_SHORT_TREND]:
        lim = limits_for(
            _alloc(regime=MarketRegime.CHOP),
            reason=reason,
            equity_usd=10_000,
            gross_exposure_usd=0,
        )
...
def test_max_gross_leverage_cap_enforced():
    lim = limits_for(
        _alloc(max_lev=1.5, stable=0.3),
        reason=ReasonCode.ENTER_LONG_TREND,
        equity_usd=10_000,
        gross_exposure_usd=8_000,
    )
...
def test_exposure_exhaustion_blocks_new_entry():
    lim = limits_for(
        _alloc(max_lev=2.0, stable=0.2),
        reason=ReasonCode.ENTER_LONG_TREND,
        equity_usd=10_000,
        gross_exposure_usd=16_000,
    )
    assert lim.allowed is False
